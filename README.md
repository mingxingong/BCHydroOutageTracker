...
## 5. View the hotspot map
`heatmap.html` is a self-contained page (Leaflet + Leaflet.heat + Leaflet.markercluster, all loaded from a CDN, no build step) with two toggleable layers over Metro Vancouver:

- **Hotspot heat** — `outage_hotspot_grid` as a heatmap: the aggregate polygon-overlay score per ~1km cell, no per-outage detail, good for "which area gets hit most" at a glance.
- **Outage clusters** — one point per row in `outage_events` (using its reported lat/lon), grouped into numbered bubbles by proximity. Bubbles get bigger as more outages roll up into them; zoom in and they split apart into smaller clusters, down to individual pins. Click any pin for that outage's municipality, cause, customers out, and off/on times with computed duration. This is the layer that answers "how many outages, and when/how long" for a given area — the heat layer alone doesn't carry that detail.
...
3. Paste both into the bar at the top and click **Load**. Uncheck "include active outages" to limit the cluster layer to resolved history only.
...
Note: clustering is done on each outage's reported point location, not its actual polygon boundary — drawing thousands of overlapping historical polygons directly would be unreadable. If you want the exact BC Hydro-drawn affected-area shape for one outage, that's a small addition (fetch `st_asgeojson(polygon)` for that row on marker click) - ask if you want it wired up.
